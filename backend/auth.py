    ...
    resend.Emails.send(
        {
            "from": sender,
            "to": [email],
            "subject": "Your RAG Chat verification code",
            "text": (
                f"Your RAG Chat verification code is {otp}.\n\n"
...
def verify_otp():
    ...
    if not check_password_hash(record.otp_hash, code):
        db.session.commit()
...
def google_callback():
    ...
    if not email or not google_id:
        return redirect(
            _frontend_url(
                "/login?error=google_missing_identity"
            )
        )
